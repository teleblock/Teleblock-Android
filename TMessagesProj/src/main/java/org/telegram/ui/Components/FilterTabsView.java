...
public class FilterTabsView extends FrameLayout {
    ...
    private float rad = AndroidUtilities.dpf2(20);
    // 自定义END
    ...
    public class Tab {
        ...
        public int getWidth(boolean store) {
            ...
            if (c > 0) {
                ...
                countWidth=AndroidUtilities.dp(20);
            }
    ...
    public class TabView extends View {
        ...
        // 自定义START
        private GradientDrawable noSelectorDrawable;
        // 自定义END
        ...
        public TabView(Context context) {
            super(context);
            noSelectorDrawable = new GradientDrawable(GradientDrawable.Orientation.LEFT_RIGHT, null);
            noSelectorDrawable.setCornerRadii(new float[]{rad, rad, rad, rad, rad, rad, rad, rad});
            noSelectorDrawable.setColor(Color.parseColor("#F7F8F9"));
        }
        ...
        protected void onDraw(Canvas canvas) {
            ...
            if (animateToKey == null) {
                int color1= otherKey.equals(activeTextColorKey) ?Color.WHITE:Theme.getColor(otherKey);
                int color2= key.equals(activeTextColorKey) ?Color.WHITE:Theme.getColor(key);
                if ((animatingIndicator || manualScrollingToId != -1) && (currentTab.id == id1 || currentTab.id == id2)) {
                    textPaint.setColor(ColorUtils.blendARGB(color1,color2 , animatingIndicatorProgress));
                } else {
            ...
            if (countWidth != 0) {
                ...
                tabWidth += AndroidUtilities.dp(20);
            }

            if (currentTab.id!=selectedTabId) {
                noSelectorDrawable.setAlpha((int) (255 * listView.getAlpha()));
                float indicatorX = (getMeasuredWidth() - tabWidth) / 2f;
                noSelectorDrawable.setBounds((int) indicatorX, AndroidUtilities.dpr(10), (int) (indicatorX + tabWidth), getMeasuredHeight() - AndroidUtilities.dpr(10));
                noSelectorDrawable.draw(canvas);
            }
...
//                    int color1 = Theme.getColor(unreadKey);
                    int color1= unreadKey.equals(Theme.key_chats_tabUnreadActiveBackground) ?Color.WHITE:Color.parseColor("#707070");
                    if ((animatingIndicator || manualScrollingToPosition != -1) && (currentTab.id == id1 || currentTab.id == id2)) {
//                        int color3 = Theme.getColor(unreadOtherKey);
                        int color3= unreadOtherKey.equals(Theme.key_chats_tabUnreadActiveBackground) ?Color.WHITE:Color.parseColor("#707070");
                ...
                if (counterPaint.getColor()==Color.WHITE){
                    textCounterPaint.setColor(getResources().getColor(R.color.theme_color));
                }
    ...
    private SparseIntArray positionCountWidth = new SparseIntArray(5);
    ...
    public FilterTabsView(Context context, BaseFragment fragment) {
        super(context);
...
//        selectorDrawable.setColor(Theme.getColor(tabLineColorKey));
        selectorDrawable.setColor(getResources().getColor(R.color.theme_color));
        ...
        listView.setPadding(AndroidUtilities.dp(0), 0, AndroidUtilities.dp(7), 0);
...
//        addView(scrollView, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.MATCH_PARENT));
        frameLayout = new FrameLayout(getContext()) {
            ...
            protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
                return FilterTabsView.this.drawChild(canvas, child, drawingTime);
            }
        };
        addView(frameLayout, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, LayoutHelper.MATCH_PARENT));
        frameLayout.addView(listView,LayoutHelper.createLinear(LayoutHelper.MATCH_PARENT, LayoutHelper.MATCH_PARENT,Gravity.CENTER_VERTICAL,66,0,0,0));
        ImageView imageView=new ImageView(getContext());
        imageView.setImageResource(R.drawable.btn_add_group_normal);
        imageView.setBackgroundColor(Theme.getColor(Theme.key_windowBackgroundWhite));
        imageView.setPadding(AndroidUtilities.dp(16), 0, AndroidUtilities.dp(10), 0);
        frameLayout.addView(imageView,LayoutHelper.createFrame(66, LayoutHelper.MATCH_PARENT,Gravity.CENTER_VERTICAL));
        imageView.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                EventUtil.track(getContext(), EventUtil.Even.信息分组加号点击, new HashMap<>());
    ...
    public void setAnimationIdicatorProgress(float value) {
        if ((int) value == 0 || (int) value == 1) {
            animatingIndicatorProgress = 1; // 点击切换tab直接变1
        }
        listView.invalidateViews();
        invalidate();
        if (delegate != null) {
    ...
    private void updateTabsWidths() {
        ...
        for (int a = 0, N = tabs.size(); a < N; a++) {
            ...
            positionCountWidth.put(a, tabs.get(a).countWidth);
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        if (child == listView) {
            ...
            if (/*animatingIndicator || manualScrollingToPosition != -1*/false) {
                ...
                if (position != RecyclerListView.NO_POSITION) {
                    ...
                    if (holder != null) {
                        ...
                        float prevX = positionToX.get(idx1)-(float)positionCountWidth.get(idx1)/2;
                        float newX = positionToX.get(idx2)-(float)positionCountWidth.get(idx2)/2;
                        int prevW = positionToWidth.get(idx1)+positionCountWidth.get(idx1);
                        int newW = positionToWidth.get(idx2)+positionCountWidth.get(idx2);
            ...
            indicatorX+=AndroidUtilities.dp(66);
            if (indicatorWidth != 0) {
                canvas.save();
...
//                selectorDrawable.setBounds((int) indicatorX, height - AndroidUtilities.dpr(4), (int) (indicatorX + indicatorWidth), height);
                selectorDrawable.setBounds((int) indicatorX, AndroidUtilities.dpr(10), (int) (indicatorX + indicatorWidth), height-AndroidUtilities.dpr(10));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        if (!tabs.isEmpty()) {
            int width = MeasureSpec.getSize(widthMeasureSpec) - AndroidUtilities.dp(66) - AndroidUtilities.dp(7);
