...
import teleblock.widget.NftBorderView;

public class BackupImageView extends View {

    // 自定义START
    private boolean drawNftBorder;
    private NftBorderView nftBorderView;
    // 自定义END
    ...
    public void setForUserOrChat(TLObject object, AvatarDrawable avatarDrawable) {
        ...
        if (object instanceof TLRPC.User) {
            ...
            if (isUserSelf && user.photo != null) {
                WalletInfo walletInfo = MMKVUtil.getWalletInfo();
                if (walletInfo != null) {
                    drawNftBorder = walletInfo.getNft_photo_id() == user.photo.photo_id;
                    if (drawNftBorder) {
                        if (nftBorderView == null) {
                            nftBorderView = new NftBorderView(getContext());
                        }
                        nftBorderView.setNftData(walletInfo.nft_name);
                    }
                }
            }
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (nftBorderView != null) {
            nftBorderView.measure(widthMeasureSpec, heightMeasureSpec);
        }
    }
