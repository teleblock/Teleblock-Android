 ...
 * 描述：频道内容页面
 */
public class ContentTabView extends FrameLayout {
    private ViewTabContentBinding binding;

    private BaseFragment mBaseFragment;
    private List<ChannelTagEntity> tagList = new ArrayList<>();
    private List<View> pageFragmentView = new ArrayList<>();
    ...
    public ContentTabView(@NonNull DialogsActivity dialogsActivity) {
    ...
    private void initView() {
        ...
        //初始化适配器
        mTgFragmentVp2Adapter = new TgFragmentVp2Adapter(new ArrayList<>());
    ...
    private void getChannelTag() {
        ...
        for (int i = 0; i < tagList.size(); i++) {
            ...
            ChannelFeedFragment pageFragment = new ChannelFeedFragment(mBaseFragment, entity, "HomeChannelTab");
        ...
        //设置适配器
        mTgFragmentVp2Adapter.setList(pageFragmentView);
        //vp默认页面
    ...
    public void onEventInfo(MessageEvent messageEvent) {
        switch (messageEvent.getType()) {
            case EventBusTags.CHANNEL_TAG_REFRASH:
                boolean force = (boolean) messageEvent.getData();
                //标签栏有变化
                if (force) {
                    mTgFragmentVp2Adapter.setList(new ArrayList<>());
                }
                getChannelTag();
                break;
